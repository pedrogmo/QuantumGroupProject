...
def assert_bitstring(bitstring: str) -> None:
...
def assert_package_length(package_length: int) -> None:
...
def bell_state(circuit: QuantumCircuit, index_0: int, index_1: int) -> None:
    circuit.h(index_0)
    circuit.cx(index_0, index_1)


def bell_state_inv(circuit: QuantumCircuit, index_0: int, index_1: int) -> None:
    circuit.cx(index_0, index_1)
    circuit.h(index_0)


def encode_bit_pair(circuit: QuantumCircuit, bit_pair: str, index: int) -> None:
    if bit_pair[0] == "1":
        circuit.z(index)
    if bit_pair[1] == "1":
        circuit.x(index)
